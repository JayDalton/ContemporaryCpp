...
namespace video {
   ...
   consteval auto to_underlying(PixelFormat format) 
   {
      return static_cast<std::underlying_type_t<PixelFormat>>(format);
   }

   consteval auto FormatBits() 
   {
	   return std::bit_width(to_underlying(PixelFormat::_largest));
   }
   ...
   {
      auto Filler               = noFrame;
      Filler.Header_.Timestamp_ = std::chrono::duration_cast<FrameHeader::µSeconds>(Duration);
      return Filler;
   }
